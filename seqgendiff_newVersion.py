...
def generate_poisson_dataset(input_csv, output_csv, noise_std, hasControl, graphml_path, number_sample):
    ...
    counts = np.array([safe_poisson(2**Theta) for _ in range(number_sample)])
    print(counts.shape)
    
    if hasControl:
        # Create combined dataset
        tilde_counts = np.array([safe_poisson(2**Theta_perturbed) for _ in range(number_sample)])
        data_matrix = np.hstack((counts.T, tilde_counts.T))

        # Create sample labels
        labels = ['Control'] * number_sample + ['Treatment'] * number_sample
...
generate_poisson_dataset(input_csv, output_csv, 1, True, graphml_path, 500)
